...
public class Platform : MonoBehaviour
{
    ...
    {
        if(contact.gameObject.tag == "Ball" && BallIsActiv._ballIsActive)
        {
            ...
            {
                ballRb.AddForce(new Vector2(-(Mathf.Abs(0.005f * PowerOfBall._powerBall)), 0.055f * PowerOfBall._powerBall));
                Debug.Log("1-" + difference);
            }
            if (difference > _lengthOfAppliedForce && difference < _lengthOfAppliedForce * 2)
            {
                ballRb.AddForce(new Vector2(-Mathf.Abs(0.01f * PowerOfBall._powerBall), 0.05f * PowerOfBall._powerBall));
                Debug.Log("2-" + difference);
            }
            if (difference > _lengthOfAppliedForce * 2 && difference < _lengthOfAppliedForce*3)
            {
                ballRb.AddForce(new Vector2(-(Mathf.Abs(0.02f * PowerOfBall._powerBall)), 0.04f * PowerOfBall._powerBall));
                Debug.Log("3-" + difference);
            }
            if (difference > _lengthOfAppliedForce * 3 && difference < _lengthOfAppliedForce * 4)
            {
                ballRb.AddForce(new Vector2(-(Mathf.Abs(0.03f * PowerOfBall._powerBall)), 0.03f * PowerOfBall._powerBall));
                Debug.Log("4-" + difference);
            }
            if (difference > _lengthOfAppliedForce * 4 && difference < _lengthOfAppliedForce * 5)
            {
                ballRb.AddForce(new Vector2(-(Mathf.Abs(0.04f * PowerOfBall._powerBall)), 0.02f * PowerOfBall._powerBall));
                Debug.Log("5-" + difference);
            }
            if (difference > _lengthOfAppliedForce * 5 && difference < _lengthOfAppliedForce * 6)
            {
                ballRb.AddForce(new Vector2(-(Mathf.Abs(0.05f * PowerOfBall._powerBall)), 0.01f * PowerOfBall._powerBall));
                Debug.Log("6-" + difference);
            }
            if (difference > _lengthOfAppliedForce * 6 && difference < _lengthOfAppliedForce * 7.1)
            {
                ballRb.AddForce(new Vector2(-(Mathf.Abs(0.055f * PowerOfBall._powerBall)), 0.005f * PowerOfBall._powerBall));
